<div class="userpro userpro-<?php echo $i; ?> userpro-id-<?php echo $user_id; ?> userpro-<?php echo $layout; ?>" <?php userpro_args_to_data( $args ); ?>>
	...
	<?php $i = 0; if (isset($user_request) && $user_request != '' ) : ?>
			
	<?php $user_request = array_reverse($user_request, true); foreach($user_request as $user=>$arr) : $userdata = get_userdata($user); if ($userdata) { $i++; ?>
	...
	<div class="userpro-body userpro-body-nopad">
		...
		?>
		<?php $i = 0; if (isset($approve_userlist) && $approve_userlist != '' ) : ?>
...
{
	$i++;
	if($i>$offset && $i<=($offset+$args['connection_per_page'])){
		$arrfollowers[$k] =1 ;
}
}
...
?>		
		...
		<?php $user_request = array_reverse($approve_userlist, true); foreach($approve_userlist as $user=>$arr) : $userdata = get_userdata($user); if ($userdata) { $i++; ?>
		<div class="userpro-sc" id="<?php echo $user; ?>">
			<div class="userpro-sc-img" data-key="profilepicture">
				<a href="<?php echo $userpro->permalink( $user ); ?>"><?php echo get_avatar( $user, 40 ); ?></a>
			</div>
