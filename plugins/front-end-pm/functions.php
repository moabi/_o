...
function fep_format_date( $date, $d ) {
	...
	if ( is_admin() || ! in_array( get_post_type(), apply_filters( 'fep_post_types_for_time', array(
			'fep_message',
			'fep_announcement'
		) ) )
	) {
		...
		if ( strlen( $searchq ) > 0 ) {
			...
			if ( ! empty( $user_query->results ) ) {
				foreach ( $user_query->results as $user ) {
					...
					?>
					<li><a href="#"
					       onClick="fep_fill_autosuggestion('<?php echo $user->user_nicename; ?>','<?php echo $user->display_name; ?>');return false;"><?php echo $user->display_name; ?></a>
					</li>
					<?php
...
function fep_map_meta_cap( $caps, $cap, $user_id, $args ) {
	...
	$our_caps = array(
		...
		'edit_fep_message',
		'delete_fep_message',
		'read_fep_announcement',
		'edit_fep_announcement',
		'delete_fep_announcement'
	);
