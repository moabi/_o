...
class Fep_Message {
	...
	function actions_filters() {
		...
		add_action( 'save_post_fep_message', array(
			$this,
			'recalculate_user_stats'
		), 20 ); //after '_participants' meta saved
	 ...
	 * @param $id
	 *
	 * @return array|bool|false|int|null|WP_Post
	 */
	...
	function get_column_content( $column ) {
		switch ( $column ) {
			...
			case 'avatar' :
				...
				break;
			case 'author' :
				?><span class="fep-message-author"><?php the_author_meta( 'display_name' ); ?></span><span
				class="fep-message-date"><?php the_time(); ?></span><?php
				break;
			case 'title' :
				if ( ! fep_is_read( true ) ) {
				...
				?><span class="<?php echo $class; ?>"><a
					href="<?php echo fep_query_url( 'viewmessage', array( 'id' => get_the_ID() ) ); ?>"><?php the_title(); ?></a>
				</span><?php echo $span; ?>
				<div class="fep-message-excerpt"><?php echo fep_get_the_excerpt( 100 ); ?></div><?php
				break;
			default:
