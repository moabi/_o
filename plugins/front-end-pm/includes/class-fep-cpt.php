...
class Fep_Cpt {
	...
	function actions_filters() {
		...
		add_action( 'manage_fep_announcement_posts_custom_column', array(
			$this,
			'announcement_columns_content'
		), 10, 2 );
	...
	function add_meta_boxes() {
		...
		add_meta_box( 'fep_announcement_to', __( 'Announcement to roles', 'front-end-pm' ), array(
			$this,
			'announcement_to'
		), 'fep_announcement', 'side', 'core' );
	}
	...
	function announcement_to( $post ) {
		...
		foreach ( get_editable_roles() as $key => $role ) {
			...
			?><label><input id="" class="" name="participant_roles[]" type="checkbox"
			                value="<?php echo $key; ?>" <?php if ( in_array( $key, $participants ) ) {
				echo 'checked="checked"';
			} ?> /> <?php echo translate_user_role( $role['name'] ); ?></label><br/><?php
		}
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit' ) {
			...
			if ( $parent ) {
				...
				<input type="hidden" name="message_to" id="fep-message-to" autocomplete="off"
				       value="<?php echo fep_get_userdata( $to, 'user_login' ); ?>"/>
				<input type="text" name="message_top" id="fep-message-top" autocomplete="off"
				       value="<?php echo fep_get_userdata( $to, 'display_name' ); ?>"/>
				<img src="<?php echo FEP_PLUGIN_URL; ?>assets/images/loading.gif" class="fep-ajax-img"
				     style="display:none;"/>
	...
	function save_message( $message_id, $message, $update ) {
		if ( ! is_admin() ) {
			return;
		} //only for BACK END . for FRONT END use 'fep_action_message_after_send' action hook
		if ( empty( $message_id ) || empty( $message ) || empty( $_POST ) ) {
			return;
		}
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
			return;
		}
		if ( is_admin() && ( empty( $_POST['fep_nonce'] ) || ! wp_verify_nonce( $_POST['fep_nonce'], 'fep_nonce' ) ) ) {
			return;
		}
		if ( wp_is_post_revision( $message ) ) {
			return;
		}
		if ( wp_is_post_autosave( $message ) ) {
			return;
		}
		//if ( ! current_user_can( 'edit_fep_messages' ) ) return;
		if ( ! current_user_can( 'edit_fep_message', $message_id ) && ! current_user_can( 'delete_fep_message', $message_id ) ) {
			return;
		}
	...
	function save_announcement( $announcement_id, $announcement, $update ) {
		if ( empty( $announcement_id ) || empty( $announcement ) || empty( $_POST ) ) {
			return;
		}
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
			return;
		}
		if ( empty( $_POST['fep_nonce'] ) || ! wp_verify_nonce( $_POST['fep_nonce'], 'fep_nonce' ) ) {
			return;
		}
		if ( wp_is_post_revision( $announcement ) ) {
			return;
		}
		if ( wp_is_post_autosave( $announcement ) ) {
			return;
		}
		//if ( ! current_user_can( 'edit_fep_announcements' ) ) return;
		if ( ! current_user_can( 'edit_fep_announcement', $announcement_id ) && ! current_user_can( 'delete_fep_announcement', $announcement_id ) ) {
			return;
		}
