...
class Fep_Attachment {
	 ...
	 * @param $message_id
	 * @param $message
	 * @param $inserted_message
	 *
	 * @return bool
	 */
	public function upload_attachment( $message_id, $message, $inserted_message ) {
	 ...
	 * @return bool
	 */
	public function upload_file( $upload_data, $message_id, $inserted_message ) {
	...
	public function display_attachment() {
		...
		if ( $attachments ) {
			...
			foreach ( $attachments as $attachment ) {
				...
				echo "<a href='" . fep_query_url( 'download', array( 'id'    => $attachment_id,
				                                                     'token' => fep_create_nonce( 'download' . $attachment_id )
					) ) . "' title='Download {$name}'>{$name}</a><br />";
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function delete_attachment( $message_id ) {
